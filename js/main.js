window.onload = function(){
	var game = new Phaser.Game(480, 320, Phaser.CANVAS, "game-area");
...
function Loading(game){
	this.preload=function () {
		...
		game.load.image("over", "assets/img/gameover3.png");
		game.load.image("over-2", "assets/img/gameover2.png");
		game.load.image('star', "assets/img/star.png");
		game.load.spritesheet('lei', "assets/img/boom.png",25,25);
		...
		game.load.spritesheet("bullet", "assets/img/bullet2.png", 6, 6);  
...
function Menu(game) {
	...
	this.create = function() {
		...
		sprite = game.add.sprite((game.width)/2, game.world.centerY,"title");
		sprite.anchor.set(0.5);
		game.add.tween(sprite).from( { y: -200 }, 1200, Phaser.Easing.Bounce.Out, true);
...
function Main(game){
	...
	this.create = function(){
		...
		stars = game.add.group(); //星星
		stars.enableBody = true;
		for (var i = 0; i < 12; i++)
		{
			var random= new Phaser.RandomDataGenerator;
			var star = stars.create(random.integerInRange(100,200), random.integerInRange(50,100), 'star');
			// star.body.gravity.y = 300;
			// star.body.bounce.y = 0.7 + Math.random() * 0.2;
		}
		lei1 =game.add.group();  //炸弹
		lei1.enableBody = true;
		for (var i = 0; i < 12; i++)
		{
			var random= new Phaser.RandomDataGenerator;
			var lei = lei1.create(random.integerInRange(50,200), random.integerInRange(100,200), 'lei');
			// star.body.gravity.y = 300;
			// star.body.bounce.y = 0.7 + Math.random() * 0.2;
		}
		lei2 =game.add.group();  //炸弹
		lei2.enableBody = true;
		for (var i = 0; i < 12; i++)
		{
			var random= new Phaser.RandomDataGenerator;
			var lei = lei2.create(random.integerInRange(50,200), random.integerInRange(200,300), 'lei');
			// star.body.gravity.y = 300;
			// star.body.bounce.y = 0.7 + Math.random() * 0.2;
		}
		lei3 =game.add.group();  //炸弹
		lei3.enableBody = true;
		for (var i = 0; i < 12; i++)
		{
			var random= new Phaser.RandomDataGenerator;
			var lei = lei3.create(random.integerInRange(200,300), random.integerInRange(50,80), 'lei');
			// star.body.gravity.y = 300;
			// star.body.bounce.y = 0.7 + Math.random() * 0.2;
		}
	...
	this.update = function(){
		...
		game.physics.arcade.overlap(player, lei1, this.fireHit, null, this);
		game.physics.arcade.overlap(player, lei2, this.fireHit, null, this);
		game.physics.arcade.overlap(player, lei3, this.fireHit, null, this);
	...
	this.collectStar=function(player, star) {
		...
		for (var i=0;i<10;i++){  //这里限制发弹数量
			var fire = myFires.create(0,0,"bullet",0).kill();
			fire.checkWorldBounds = true;
			fire.outOfBoundsKill = true;
		}
		myFires.createMultiple(80, 'bullet');
        star.kill();
	};
	...
	this.gameOver = function(){
		...
		game.add.tween(over).to({y:304},1000,"Linear",true).onComplete.add(function(){
			...
			sprite = game.add.sprite(game.world.centerX, -200, 'over');
			sprite.anchor.set(0.5);
			game.add.tween(sprite).to( { y: game.world.centerY }, 1000, Phaser.Easing.Bounce.Out, true);
